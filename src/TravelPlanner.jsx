...
function exportTripHTML(trip) {
  ...
  const days = trip.days.map((day, i) => {
    const items = (day.schedule || []).map(s => `<li>${s.time ? `<b>${s.time}</b> ` : ""}${s.text}${s.done ? " ✓" : ""}${s.memo ? `<div style="color:#888;font-size:13px;margin-top:4px">${s.memo}</div>` : ""}</li>`).join("");
    const memo = day.memo ? `<p style="color:#666;font-size:14px;line-height:1.8;border-top:1px solid #f0f0f0;padding-top:12px;margin-top:12px">${day.memo.replace(/\n/g, "<br>")}</p>` : "";
    return `<div style="background:#fff;border-radius:14px;padding:20px;margin:16px 0;border-left:4px solid ${color}"><h2 style="color:#2d3748;font-size:17px;margin:0 0 12px">Day ${i+1}${day.date ? ` · ${formatDate(day.date)}` : ""}${day.title ? ` — ${day.title}` : ""}</h2>${items ? `<ul style="margin:0;padding-left:20px;line-height:2.2">${items}</ul>` : ""}${memo}</div>`;
  }).join("\n");
  const html = `<!DOCTYPE html><html lang="ko"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${trip.emoji} ${trip.title}</title><style>body{font-family:'Apple SD Gothic Neo',sans-serif;max-width:680px;margin:0 auto;padding:20px 16px;color:#2d3748;background:#f9fafb}h1{color:${color};font-size:26px}</style></head><body><h1>${trip.emoji} ${trip.title}</h1><p style="color:#999;font-size:13px">${trip.days.length}일 여행</p>${days}</body></html>`;
  const blob = new Blob([html], { type: "text/html" });
  const url = URL.createObjectURL(blob);
  const a = document.createElement("a"); a.href = url; a.download = trip.title + ".html"; a.click();
  URL.revokeObjectURL(url);
}
function exportAllJSON(trips) {
  const blob = new Blob([JSON.stringify({ trips }, null, 2)], { type: "application/json" });
  const url = URL.createObjectURL(blob);
  const a = document.createElement("a"); a.href = url; a.download = "all_travels.json"; a.click();
  URL.revokeObjectURL(url);
}
        ...
        : <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(120px, 1fr))", gap: 6 }}>
            {photos.map(p => (
              <div key={p.id} style={{ position: "relative", aspectRatio: "1", borderRadius: 10, overflow: "hidden" }}>
                ...
                <button onClick={() => onDelete(p.id)} style={{ position: "absolute", top: 4, right: 4, background: "rgba(0,0,0,0.5)", border: "none", borderRadius: "50%", width: 22, height: 22, color: "#fff", cursor: "pointer", fontSize: 11 }}>✕</button>
              </div>
            ))}
          </div>
      }
...
function Memo({ value, onChange }) {
  return (
    <div style={{ padding: "0 16px 16px" }}>
      <textarea value={value} onChange={e => onChange(e.target.value)} placeholder="메모를 자유롭게 입력하세요..."
      ...
      {editing ? (
        <div style={{ padding: "10px 12px" }}>
          <div style={{ display: "flex", gap: 8, marginBottom: 8, alignItems: "center" }}>
            <input type="time" value={editTime} onChange={e => setEditTime(e.target.value)} style={S.input({ width: 100, flexShrink: 0, fontSize: 13 })} />
            <input value={editText} onChange={e => setEditText(e.target.value)} onKeyDown={e => e.key === "Enter" && saveEdit()} autoFocus style={S.input({ flex: 1, fontSize: 13 })} />
          </div>
          <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
            ...
            <button onClick={saveEdit}
              style={{ padding: "5px 14px", border: "none", borderRadius: 8, background: accent || "#667eea", color: "#fff", fontSize: 12, cursor: "pointer", fontWeight: 600 }}>저장</button>
          </div>
        </div>
      ) : (
    ...
    <div style={{ padding: "0 16px 16px" }}>
      ...
      {items.length === 0
        ...
        : <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {items.map((item, idx) => (
              <ScheduleItem key={item.id} item={item} idx={idx} total={items.length} accent={accent}
...
function TripCard({ trip, onOpen, onDelete }) {
  ...
  return (
    <div style={{ borderRadius: 18, overflow: "hidden", marginBottom: 14, boxShadow: "0 4px 20px rgba(0,0,0,0.10)", cursor: "pointer" }} onClick={() => onOpen(trip.id)}>
      <div style={{ background: col.bg, padding: "20px 18px 16px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
          <div>
            ...
            {firstDate && <p style={{ margin: "4px 0 0", color: "rgba(255,255,255,0.8)", fontSize: 12 }}>{formatDateShort(firstDate)}{lastDate && lastDate !== firstDate ? ` ~ ${formatDateShort(lastDate)}` : ""}</p>}
          </div>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <button onClick={() => { const e = TRIP_EMOJIS; updateTrip({ emoji: e[(e.indexOf(trip.emoji) + 1) % e.length] }); }}
                ...
                {editingTitle
                  ...
                  : <h1 onClick={() => setEditingTitle(true)} style={{ margin: 0, color: "#fff", fontSize: 18, fontWeight: 700, cursor: "pointer" }}>
                      {trip.title} <span style={{ fontSize: 13, opacity: 0.7 }}>✏️</span>
                    </h1>
                }
              </div>
              <p style={{ margin: "3px 0 0", color: "rgba(255,255,255,0.75)", fontSize: 12 }}>
                {trip.days.length}일 여행{trip.days[0].date ? " · " + formatDateShort(trip.days[0].date) + " 출발" : ""}
              </p>
            </div>
            ...
            <div style={{ background: "rgba(0,0,0,0.2)", borderRadius: 14, padding: 14, marginBottom: 12 }}>
              ...
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                ...
                <button onClick={() => {
                  ...
                  const url = URL.createObjectURL(blob);
                  const a = document.createElement("a"); a.href = url; a.download = trip.title + ".json"; a.click();
            ...
            {trip.days.map((day, i) => {
              ...
              return (
                <button key={day.id} onClick={() => setActiveDay(i)}
                  style={{ flexShrink: 0, padding: "7px 14px", borderRadius: 20, border: "2px solid " + (active ? "#fff" : "rgba(255,255,255,0.35)"), background: active ? "rgba(255,255,255,0.25)" : "transparent", color: "#fff", fontSize: 12, fontWeight: 600, cursor: "pointer", transition: "all 0.15s" }}>
                  Day {i + 1}
                  {day.date && <span style={{ display: "block", fontSize: 10, fontWeight: 400, opacity: 0.85 }}>{new Date(day.date + "T00:00:00").toLocaleDateString("ko-KR", { month: "numeric", day: "numeric" })}</span>}
                </button>
              );
            })}
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                  <input type="date" value={cur.date} onChange={e => updateDay(activeDay, { date: e.target.value })}
                  ...
                  {activeDay === 0 && cur.date && <span style={{ fontSize: 11, color: "#bbb" }}>← 설정 시 이후 날짜 자동 업데이트</span>}
                </div>
              ...
              {trip.days.length > 1 && <button onClick={() => removeDay(activeDay)} style={S.ghost({ fontSize: 18 })}>🗑</button>}
            </div>
            <div style={{ display: "flex" }}>
              {[["schedule","📅 일정"],["photos","🖼 사진"],["memo","📝 메모"]].map(([key, label]) => (
                <button key={key} onClick={() => setActiveTab(key)}
                  style={{ flex: 1, padding: "10px 0", border: "none", background: "none", fontSize: 13, fontWeight: 600, cursor: "pointer", color: activeTab === key ? col.accent : "#bbb", borderBottom: "2px solid " + (activeTab === key ? col.accent : "transparent"), transition: "all 0.2s" }}>{label}</button>
              ))}
            </div>
          </div>
          <div style={{ paddingTop: 16 }}>
            {activeTab === "schedule" && <Schedule items={cur.schedule} accent={col.accent} onChange={v => updateDay(activeDay, { schedule: v })} />}
            {activeTab === "photos" && <PhotoGrid photos={cur.photos || []} onAdd={p => updateDay(activeDay, { photos: [...(cur.photos || []), p] })} onDelete={id => updateDay(activeDay, { photos: (cur.photos || []).filter(p => p.id !== id) })} />}
            {activeTab === "memo" && <Memo value={cur.memo} onChange={v => updateDay(activeDay, { memo: v })} />}
          </div>
        </div>
      </div>
    </div>
...
function Home({ trips, onOpen, onAdd, onDelete, sheetUrl, setSheetUrl }) {
  ...
  const handleLoad = async () => {
    ...
    if (d?.trips) { window._setTrips(d.trips); setSyncStatus("✅ 불러오기 완료!"); }
...
export default function App() {
  ...
  useEffect(() => {
    try {
      const s = localStorage.getItem(STORAGE_KEY); if (s) setTrips(JSON.parse(s));
      const su = localStorage.getItem(SHEET_URL_KEY); if (su) setSheetUrl(su);
    } catch {}
  }, []);
  useEffect(() => { try { localStorage.setItem(STORAGE_KEY, JSON.stringify(trips)); } catch {} }, [trips]);
  useEffect(() => { try { if (sheetUrl) localStorage.setItem(SHEET_URL_KEY, sheetUrl); } catch {} }, [sheetUrl]);
  useEffect(() => {
    try {
  ...
  return <Home trips={trips} onOpen={setOpenTripId} onAdd={addTrip} onDelete={deleteTrip} sheetUrl={sheetUrl} setSheetUrl={setSheetUrl} />;
}

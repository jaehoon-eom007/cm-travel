...
function exportHTML(trip) {
  ...
  const days = trip.days.map((d,i) => {
    const items = (d.schedule||[]).map(s =>
      `<li>${s.time?`<b>${s.time}</b> `:""}${s.text}${s.done?" ✓":""}${s.memo?`<p style="margin:2px 0;color:#888;font-size:12px">${s.memo}</p>`:""}</li>`
    ).join("");
...
function PhotoGrid({ photos, onAdd, onDel }) {
  ...
  const read = files => Array.from(files).forEach(f => {
    ...
    const r = new FileReader(); r.onload = e => onAdd({id:uid(),src:e.target.result,name:f.name}); r.readAsDataURL(f);
  });
  useEffect(() => {
    const h = e => { for (const it of (e.clipboardData?.items||[])) if (it.type.startsWith("image/")) { const r=new FileReader(); r.onload=ev=>onAdd({id:uid(),src:ev.target.result,name:"클립보드"}); r.readAsDataURL(it.getAsFile()); } };
...
function Schedule({ items, ac, onChange }) {
  const [text,setText]=useState(""); const [time,setTime]=useState("");
  const add=()=>{ if(!text.trim())return; onChange([...items,{id:uid(),time,text:text.trim(),done:false,memo:""}]); setText(""); setTime(""); };
  const move=(idx,dir)=>{ const a=[...items],t=idx+dir; if(t<0||t>=a.length)return; [a[idx],a[t]]=[a[t],a[idx]]; onChange(a); };
  return (
    <div style={{padding:"0 16px 16px"}}>
    ...
    <div style={{marginBottom:28}}>
      <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:12}}>
        ...
        <div style={{flex:1}}>
          ...
          <input value={day.title} onChange={e=>set({title:e.target.value})} placeholder="이 날의 테마 (예: 버킹엄 투어 🏰)"
  ...
  return (
    <div style={{minHeight:"100vh",background:"#f7f8fc",fontFamily:"inherit"}}>
        ...
        <div style={{maxWidth:760,margin:"0 auto"}}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            ...
            <div style={{flex:1}}>
              <div style={{display:"flex",alignItems:"center",gap:8}}>
                <button onClick={()=>{const e=EMOJIS;upTrip({emoji:e[(e.indexOf(trip.emoji)+1)%e.length]});}} style={{background:"none",border:"none",fontSize:22,cursor:"pointer"}}>{trip.emoji}</button>
                {editTitle
                  ?<input autoFocus value={trip.title} onChange={e=>upTrip({title:e.target.value})} onBlur={()=>setEditTitle(false)} onKeyDown={e=>e.key==="Enter"&&setEditTitle(false)}
                  ...
                  :<h1 onClick={()=>setEditTitle(true)} style={{margin:0,color:"#fff",fontSize:18,fontWeight:700,cursor:"pointer"}}>{trip.title} <span style={{fontSize:13,opacity:0.7}}>✏️</span></h1>
                }
              </div>
              <p style={{margin:"2px 0 0",color:"rgba(255,255,255,0.75)",fontSize:12}}>{trip.days.length}일 여행{trip.days[0].date?" · "+fmts(trip.days[0].date)+" 출발":""}</p>
            </div>
          ...
          {showColor&&(
            <div style={{background:"rgba(0,0,0,0.2)",borderRadius:14,padding:12,marginTop:12,display:"flex",gap:10,flexWrap:"wrap"}}>
              {PALETTE.map((c,i)=>(
                <button key={i} onClick={()=>{upTrip({colorIdx:i});setShowColor(false);}} style={{width:36,height:36,borderRadius:"50%",border:trip.colorIdx===i?"3px solid #fff":"3px solid transparent",background:c.bg,cursor:"pointer"}} />
              ))}
            </div>
          )}
          {showExport&&(
            <div style={{background:"rgba(0,0,0,0.2)",borderRadius:14,padding:14,marginTop:12}}>
              ...
              <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                ...
                <button onClick={()=>Object.assign(document.createElement("a"),{href:URL.createObjectURL(new Blob([JSON.stringify(trip,null,2)],{type:"application/json"})),download:trip.title+".json"}).click()} style={S.btn("rgba(255,255,255,0.25)")}>💾 JSON</button>
              </div>
            </div>
          )}
        </div>
      ...
      <div style={{maxWidth:760,margin:"0 auto",padding:"24px 16px 100px"}}>
        {trip.days.map((day,i)=>(
          <DaySection key={day.id} day={day} dayIdx={i} ac={col.ac} onUpdate={upDay} onRemove={removeDay} showRemove={trip.days.length>1} />
        ))}
    ...
    <div style={{borderRadius:18,overflow:"hidden",boxShadow:"0 4px 20px rgba(0,0,0,0.10)",cursor:"pointer"}} onClick={()=>onOpen(trip.id)}>
      <div style={{background:col.bg,padding:"20px 18px 16px"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
          ...
          <button onClick={e=>{e.stopPropagation();if(confirm("이 여행을 삭제할까요?"))onDel(trip.id);}} style={{background:"rgba(0,0,0,0.2)",border:"none",borderRadius:8,color:"#fff",padding:"4px 8px",cursor:"pointer",fontSize:13}}>🗑</button>
        </div>
      </div>
...
function Home({ trips, setTrips, onOpen, syncMsg }) {
  ...
  const handleSave=async()=>{
    ...
    setSyncing(false); toast(r.msg);
  };
  const handleLoad=async()=>{
    setSyncing(true); toast("불러오는 중...");
    const r=await dbLoad();
    setSyncing(false);
    if(r.ok&&r.data){setTrips(r.data);toast(r.msg);}
    else toast(r.msg);
  };
  const importJSON=e=>{
          ...
          {showSettings&&(
            <div style={{background:"rgba(0,0,0,0.2)",borderRadius:14,padding:14,marginTop:14}}>
              <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10}}>
                <div style={{width:10,height:10,borderRadius:"50%",background:isReady?"#48bb78":"#fc8181",flexShrink:0}} />
                <p style={{margin:0,color:"#fff",fontSize:13,fontWeight:600}}>
                  {isReady?"✅ Supabase 연결됨":"⚠️ Supabase 미연결 — .env 파일을 확인하세요"}
                </p>
              </div>
              <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:8}}>
                <button onClick={handleSave} disabled={!isReady||syncing} style={S.btn(isReady?"#38a169":"#a0aec0")}>☁️ 저장</button>
                <button onClick={handleLoad} disabled={!isReady||syncing} style={S.btn(isReady?"#3182ce":"#a0aec0")}>📥 불러오기</button>
                <button onClick={()=>Object.assign(document.createElement("a"),{href:URL.createObjectURL(new Blob([JSON.stringify({trips},null,2)],{type:"application/json"})),download:"travels.json"}).click()} style={S.btn("#9f7aea")}>💾 JSON 저장</button>
